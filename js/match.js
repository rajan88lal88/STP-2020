...
//converting objects to excel tuples and vice versa
...
//Driver function for getting HTML DOM of the match result page
function matchHandler(url, folder) {
  dir = folder;
  request(url, cb);
}
...
function cb(err, header, body) {
  ...
  if (err == null && header.statusCode == 200) {
    ...
    //function to scrape data for a match
    parseHtml(body);
    // fs=> file system
  ...
  for (let i = 0; i < bothInnings.length; i++) {
    ...
    for (let j = 0; j < AllRows.length; j++) {
      ...
      if (isPlayer) {
        ...
        //function to handle each players data
        processPlayer(
          teamName,
          pName,
          result,
...
function processPlayer(
  ...
  let playerFile = path.join(dir, teamPath, name) + ".xlsx";
  //reads exixting data of a player if exist
  let fileData = excelReader(playerFile, name);
  let json = fileData;
  if (fileData == null) {
    json = [];
  }
  //appending current data to excel read data
  json.push(obj);
  //write all the data to the excel
